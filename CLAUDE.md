...
- **MANDATORY**: all Python commands, scripts, and inline code must run through the project's uv-managed virtual environment. Use `uv run python` (or `uv run python -X utf8` for tools) — never invoke `python3`, `python`, `.venv/bin/python`, or any system Python directly
