...
每个 `papers/{slug}.md` 必须先完成论文类型、研究类型和研究对象分类：
...
- **Zotero 集成**：`tools/find_zotero_pdf.py`、`tools/fetch_zotero_metadata.py` 和 `tools/_zotero_snapshot.py` 支持从本地 Zotero 数据库按 `--title`、`--doi`、`--item-key` 查找 PDF 和父条目 metadata；Zotero 根目录通过 `config/zotero-roots.json` 或 `--zotero-root` 指定。`/ingest` 可利用此功能自动定位本地 PDF，并从 Zotero metadata 直接派生 plain BibTeX 字段。
