...
- one or more arXiv IDs / wiki paper slugs in the request
- `/ingest --discover` invocation (anchor = the just-ingested paper's arXiv ID)

Anchor mode is the strongest signal channel because the anchor title and identifiers constrain the no-key arXiv/Crossref search better than a broad topic string.
...
Topic mode runs no-key arXiv/Crossref search and ranks the results. It is useful for exploration when the user has a domain in mind but no specific paper in hand. `/init` is a different tool — it ingests local user-owned papers into a fresh wiki — so route topic-only requests to `/discover`, not `/init`.
