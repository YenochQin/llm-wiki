...
- `tools/init_discovery.py prepare` must pass recovered titles into `uv run python tools/prepare_paper_source.py --raw-root @raw-root --output-dir @configured-sources-papers --cache-root @mineru-cache --source <local-path> [--title "<recovered-title>"]`.
...
- The canonical prepared format is `mineru-md`: `wiki/sources/papers/<slug>.md` plus `wiki/sources/papers/assets/<slug>/<hash>.jpg` for figures.
...
- Set each local paper's `canonical_ingest_path` to the prepared `wiki/sources/papers/<slug>.md` path. If MinerU prep failed (manifest `usable: false`), record the warning and skip that paper rather than substituting the raw PDF.
