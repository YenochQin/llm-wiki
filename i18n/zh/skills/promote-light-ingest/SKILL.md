...
- Promoted pages regenerated by `/reingest` or `/ingest` must keep the fixed `## Evidence Pack` card template defined in `docs/runtime-page-templates.en.md` §papers.
- Promoted pages must write `## Related` with the fixed format in `.claude/skills/shared-references/ingest-invariants.md` §8, preserving the Summary link under `Summary` when still relevant.
