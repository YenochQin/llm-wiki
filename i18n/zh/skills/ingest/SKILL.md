...
5. Carry the Zotero-derived `bibtex` string into both `wiki/sources/papers/{slug}.md` and `wiki/papers/{slug}.md` frontmatter as a derived bibliographic field. Keep it as plain BibTeX so the three `bibbst/` styles (`gbt7714-numerical.bst`, `apsrev4-2.bst`, `elsarticle-num.bst`) can consume it directly. Do not call `tools/reference_metadata.py` from `/ingest`.
