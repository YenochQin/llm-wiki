---
name: promote-light-ingest
description: Use when the user asks which lightly-ingested background or thesis papers deserve deeper processing — ranks a promotion shortlist for upgrading light pages to full /ingest, /reingest, or /reingest-force.
argument-hint: "[--limit N] [--min-score N] [--apply <paper-slug>]"
---

# /promote-light-ingest

This skill audits paper pages created by `/ingest-light` and proposes which ones should be promoted to the full research graph. It is a proposal gate by default: it ranks candidates, explains why they matter, and gives exact upgrade commands. It does **not** batch-upgrade papers unless the user explicitly names a slug with `--apply`.

## Inputs

- `--limit N` optional, default 20: maximum candidates to show.
- `--min-score N` optional, default 0: hide low-scoring candidates.
- `--apply <paper-slug>` optional: after showing the rationale, run `/reingest @configured-sources-papers/<source-slug>.md --update-entities` for one explicitly selected light-ingest paper whose prepared source exists. If the current paper page is severely wrong and existing wiki state is likely to poison migration, prefer `/reingest-force @configured-sources-papers/<source-slug>.md`.
...
- Suggested command per paper, usually:
  ...
  - or `/reingest-force @configured-sources-papers/<source-slug>.md` when the current page should be rebuilt in clean-room mode
...
2. Prefer:
   ...
   /reingest @configured-sources-papers/<source-slug>.md --update-entities
   ```
   If the existing page is badly wrong and should not influence entity migration, use:
   ```text
   /reingest-force @configured-sources-papers/<source-slug>.md
   ```
...
- Do not create concepts, claims, people, or graph edges directly. Promotion is delegated to `/reingest`, `/reingest-force`, or `/ingest`.
- Promoted pages regenerated by `/reingest` or `/ingest` must keep the fixed `## Evidence Pack` card template: `` `E1` <UseLabel> — <short label> ...: ^E1`` with prose citations using the literal Obsidian block link `[[#^E1]]`. The outer double brackets, leading `#`, and one literal space before the citation are mandatory; do not accept `[#^E1]`, `[[^E1]]`, `#^E1`, `word[[#^E1]]`, legacy `[!E1]` markers, or cards that start with `^E1`.
...
- `/reingest` — preferred promotion action for an existing paper page when current wiki state is mostly trustworthy.
- `/reingest-force` — use when the existing paper page is badly wrong and must be rebuilt without trusting old paper-derived wiki content.
