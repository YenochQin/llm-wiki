...
- Preserve key equations in LaTeX when the source contains formulas. Prefer one or two central equations over a long dump, but any equation copied into `## Evidence Pack` must be a complete meaning-preserving unit. If the full equation is too long for a card, cite the equation label/section instead of copying a misleading partial formula. For `$$...$$` display math in Evidence Pack, only the opening `$$` line may carry the quote marker; continuation lines and the closing `$$` line must not start with `>`.
...
- `## My take` should explicitly connect the paper to the user-maintained research direction in `@configured/Summary/research-direction.md` when defensible. If the source cannot support a connection, say so in one scoped sentence rather than forcing a tie-in.
...
Before writing any graph files, verify `@configured/graph/` exists. Create it if missing.
...
- `@configured/graph/open_questions.md` rebuilt outside INIT MODE
...
The single consolidated post-ingest self-check lives in **Phase E** (`.claude/skills/shared-references/ingest-phases/phase-e-navigation-finalize.md`, "Gate E"). Run and print that checklist there; it is not duplicated here. This file supplies the per-page *floors* that Gate E verifies against — keep them in sync with Gate E if either changes.
