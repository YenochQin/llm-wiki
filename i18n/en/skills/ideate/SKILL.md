...
**Additional pre-condition steps**:
...
2. **Snapshot wiki state** (for the Growth Report at the end):
...
2. **External search** (run in parallel using Agent tool):
   ...
   - **No-key literature search**:
     ```shell
     uv run python -X utf8 tools/fetch_literature.py search "<direction-keywords>" --limit 20
     ```
...
1. **Write top ideas** (status: proposed):
   ...
   ```shell
   # generate slug
   uv run python -X utf8 tools/research_wiki.py slug "<idea-title>"
   ```
...
3. **Add graph edges**:
   ```shell
   # for each idea
   uv run python -X utf8 tools/research_wiki.py add-edge '@configured' --from "ideas/{slug}" --to "claims/{target-claim}" --type addresses_gap --evidence "Generated by ideate"

   uv run python -X utf8 tools/research_wiki.py add-edge '@configured' --from "ideas/{slug}" --to "papers/{source-paper}" --type inspired_by --evidence "Inspired by method in {paper-title}"
   ```
...
5. **Append log**:
   ```shell
   uv run python -X utf8 tools/research_wiki.py log '@configured' "ideate | {N} ideas proposed, {M} ideas filtered out | direction: {direction}"
   ```
...
### Tools
- `uv run python -X utf8 tools/research_wiki.py maturity '@configured' --json` — check wiki maturity + Growth Report
