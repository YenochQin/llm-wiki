...
- `.Codex/skills/prefill/foundations-catalog.yaml` — seed list
...
**Pre-conditions**: working directory contains `wiki/`, `tools/`, `.Codex/`. Set `WIKI_ROOT=wiki/`.
...
- **Catalog mode**: read `.Codex/skills/prefill/foundations-catalog.yaml`. Pick all entries under `domains.{domain}` plus everything under `domains.general` (general foundations apply to every research field).
...
- **Catalog file missing**: print error pointing to `.Codex/skills/prefill/foundations-catalog.yaml`.
...
### Catalog
- `.Codex/skills/prefill/foundations-catalog.yaml`
