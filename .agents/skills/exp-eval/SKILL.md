...
- `.Codex/skills/shared-references/cross-model-review.md` — reviewer independence principle
...
1. **Form Codex's independent verdict** (after reading Review LLM's verdict, Codex also analyzes independently):
   ...
   - Form Codex's own verdict and confidence suggestion
...
2. **Update experiment page**: set `outcome: inconclusive` based on the user-reported results
...
- **Only process completed experiments**: experiments with status != completed are refused; prompt user to run the experiment externally and report results before invoking /exp-eval
...
- **Experiment not completed**: report status; ask the user to run the experiment externally, paste the results into the experiment page's Results section, set `status: completed`, then re-invoke `/exp-eval {slug}`
...
- **Results data missing**: if the experiment page's Results section is empty, prompt the user to fill in metrics from their external run before re-invoking `/exp-eval`
...
### Shared References
- `.Codex/skills/shared-references/cross-model-review.md` — Review LLM independence principle (required reading)
