...
> Each round calls /review for structured feedback → parses actionable items → Codex fixes the artifact →
...
- Requires experimental validation → suggest running the experiment externally and reporting results to `/exp-eval`
...
**Category C — Claim status updates (Codex fixes wiki):**
...
### Shared References
- `.Codex/skills/shared-references/cross-model-review.md` — indirect dependency via /review
