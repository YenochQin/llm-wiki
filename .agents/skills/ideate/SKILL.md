...
1. **Write top ideas** (status: proposed):
   ...
   ```
   Create `wiki/ideas/{slug}.md` **following the AGENTS.md ideas template exactly** (all fields required; `lint.py` enforces `status` and `priority`):
   ```yaml
   ---
   title: "<idea title>"
   slug: "<idea-slug>"
   ...
   **Body sections** (exactly match the AGENTS.md template — do not rename):
   ```markdown
   ## Motivation
...
### Shared References
- `.Codex/skills/shared-references/cross-model-review.md` — Phase 2 Review LLM independence principle
