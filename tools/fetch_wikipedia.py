...
USER_AGENT = "LLMWiki-prefill/0.1 (https://github.com/skyllwt/LLMWiki)"
...
def _get(url: str) -> dict:
    ...
    try:
        ...
        if e.code == 404:
            ...
            sys.exit(2)
        print(
            json.dumps({"status": "http_error", "code": e.code, "url": url}),
            file=sys.stderr,
        )
        sys.exit(3)
    except (urllib.error.URLError, TimeoutError, json.JSONDecodeError) as e:
        print(
            json.dumps({"status": "error", "message": str(e), "url": url}),
            file=sys.stderr,
        )
        sys.exit(3)
...
def _api(params: dict) -> dict:
    ...
    if "error" in data:
        ...
        if code in ("missingtitle", "invalidtitle"):
            print(
                json.dumps({"status": "not_found", "title": params.get("page", "")}),
                file=sys.stderr,
            )
            sys.exit(2)
        print(
            json.dumps({"status": "api_error", "error": data["error"]}), file=sys.stderr
        )
        sys.exit(3)
    return data
...
def fetch_section(title: str, index: int) -> str:
    data = _api(
        {"action": "parse", "page": title, "prop": "wikitext", "section": str(index)}
    )
...
def main() -> None:
    ...
    if args.cmd == "summary":
        ...
        print(
            json.dumps(
                {"wikitext": fetch_section(args.title, args.index)}, ensure_ascii=False
            )
        )
