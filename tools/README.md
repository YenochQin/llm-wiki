...
| `prepare_zotero_source.py` | Resolve one Zotero item, persist metadata/manifest checkpoints, and prepare its PDF without agent-side argument wiring. | `uv run python -X utf8 tools/prepare_zotero_source.py --item-key ABC123 --output-dir @configured-sources-papers --cache-root @mineru-cache` |
