...
1. **Broken `[prepared markdown]` links.** `wiki/sources/papers/`, `sources/notes/`, and `sources/web/` were emptied (replaced with `.gitkeep`) at 22:45, four minutes after the 22:41 ingest finished. The concept pages had already written `[prepared markdown](../sources/papers/<slug>.md)` references to files that no longer exist. `/ingest` itself did not delete the sources, but it also did not verify the link targets at end-of-skill, so the dead links shipped silently.
